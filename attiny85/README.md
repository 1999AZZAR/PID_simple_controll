...
## Two Performance Versions Available

| Version                       | Clock Speed | Performance                 | Hardware Requirements              |
| ----------------------------- | ----------- | --------------------------- | ---------------------------------- |
| **Internal Oscillator** | 8MHz        | Standard (160Hz control)    | No external components             |
| **External Crystal**    | 20MHz       | 2.5x Faster (200Hz control) | 16MHz crystal + 2x 22pF capacitors |
...
# Switch to internal oscillator (8MHz)
./switch_config.sh internal

# Switch to external crystal (20MHz)
./switch_config.sh external
```

**Method 2: Manual Configuration**
Edit `config_common.h` and uncomment only ONE of these lines:

```cpp
#define USE_INTERNAL_OSCILLATOR  // Use 8MHz internal oscillator
//#define USE_EXTERNAL_CRYSTAL    // Use 20MHz external crystal
```

### External Crystal Hardware Setup

For 20MHz operation, add these components to your ATTiny85 board:

- **16MHz crystal** between pins 2 and 3 (PB3-PB4)
- **Two 22pF capacitors** from crystal pins to ground
- **Correct fuse settings**: Low=0xF1, High=0xDD, Extended=0xFE
